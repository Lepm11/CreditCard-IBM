...
public class CreditCardController {
	...
	/**
	 * Endpoint para mostrar la tarjeta sugerida con respecto a los parametros
	 * ingresados
	 * 
	 * @param yourPassion,            parametro de tipo string que indica la passion
	 *                                del usuario.
	 * @param monthlySalary,parametro tipo Double que indica el salario mensual del
	 *                                usuario.
	 * @param age,                    parametro de tipo Integer que indica la edad
	 *                                del usuario
	 * @return retorna la tarjeta sugeridad según los datos ingresados.
	 * @throws BadRequestException en caso de no encontrar alguna coincidencia de
	 *                             tarjeta con los datos enviados.
	 * @author Abel&Enrique
	 * 
	 * 
	 */
	@GetMapping("/sugerencia")
	public ResponseEntity<?> buscarTarjetaSugerida(@RequestParam(name = "passion") String yourPassion,
			@RequestParam(name = "salary") Double monthlySalary, @RequestParam Integer age) {
		return circuitBreaker.create("tarjetas").run(() -> {
			Optional<CreditCard> creditCard = creditCardDao.buscarTarjetaSugerida(yourPassion, monthlySalary, age);

			if (creditCard.isEmpty()) {

				throw new BadRequestException(
						String.format("No existen sugerencias para los datos ingresados, revisa los datos"));
			}
	...
	/**
	 * Endpoint para mostrar la tarjeta sugerida DTO, unicamente mostrando los datos
	 * de interés.
	 * 
	 * @param yourPassion,            parametro de tipo string que indica la passion
	 *                                del usuario.
	...
	public ResponseEntity<?> buscarTarjetaSugeridaDTO(@RequestParam(name = "passion", required = true) String yourPassion,
			@RequestParam(name = "salary",required = true) Double monthlySalary, @RequestParam(required = true) Integer age) {
			
		
			
			System.out.println(!age.getClass().getSimpleName().toString().equalsIgnoreCase("Integer"));
			System.out.println(monthlySalary.getClass().getSimpleName().toString());
			...
			return circuitBreaker.create("tarjetas").run(() -> {
				...
				List<CreditCardDTO> creditCardDto = creditCard.stream().map(CreditCardMapper::mapCreditCardDTO)
